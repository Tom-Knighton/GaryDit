...
struct PostViewPostDetails: View {
    ...
    func bottomBar() -> some View {
        VStack(alignment: .leading) {
            ...
            HStack {
                Spacer()
                PostActionButton(systemIcon: "arrow.up", tintColor: .orange, isActive: viewModel.post.postVoteStatus == .upvoted, action: { viewModel.vote(.upvoted) })
                Spacer()
                PostActionButton(systemIcon: "arrow.down", tintColor: .purple, isActive: viewModel.post.postVoteStatus == .downvoted, action: { viewModel.vote(.downvoted) })
                Spacer()
                PostActionButton(systemIcon: "bookmark", tintColor: .gray, isActive: false, action: {})
                Spacer()
                PostActionButton(systemIcon: "arrowshape.turn.up.backward", tintColor: .gray, isActive: false, action: {})
                Spacer()
                PostActionButton(systemIcon: "square.and.arrow.up", tintColor: .gray, isActive: false, action: {})
                Spacer()
            }
            Divider()
                .overlay(.primary)
