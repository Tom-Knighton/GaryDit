...
public struct PostService {
    ...
    public static func GetPostComments(for postId: String, rootId: String? = nil, sort: RedditSort = .best) async throws -> [PostComment] {
        var queryItems: [URLQueryItem] = []
        if let rootId {
            queryItems.append(URLQueryItem(name: "startFrom", value: rootId))
        }
        queryItems.append(URLQueryItem(name: "sortMethod", value: sort.rawValue))
    ...
    public static func GetMoreComments(for postId: String, replacingId: String, childIds: [String], sort: RedditSort = .best) async throws -> MoreCommentsDto {
        let dto = RequestCommentsDto(childCommentIds: childIds)
        
        let request = APIRequest(method: .post, path: "post/\(postId)/comments/\(replacingId)/more", queryItems: [URLQueryItem(name: "sortMethod", value: sort.rawValue)], body: dto.toJson())
